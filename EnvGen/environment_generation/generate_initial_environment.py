...
def call_api(log, engine="gpt-4-1106-preview", use_azure=True):
    ...
    # Older GPT-5 family models reject explicit temperature in Chat Completions.
    if not engine.lower().startswith("gpt-5"):
        request["temperature"] = 0

    resp = oai.ChatCompletion.create(**request)
...
if __name__ == "__main__":
    ...
    resp, usage = call_api(log=chatlog, engine=args.engine, use_azure=args.use_azure)
