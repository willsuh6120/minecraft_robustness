...
def get_dri_card_by_index(index):
    cards = sorted(Path("/dev/dri/by-path").glob("pci-*-card"))
    if cards:
        return os.path.realpath(str(cards[index % len(cards)]))

    fallback = Path("/dev/dri/card0")
    if fallback.exists():
...
if __name__ == "__main__":
    ...
    try:
        ...
        if num_cuda_devices == 0:
            device = get_dri_card_by_index(index)
        else:
        ...
        # Some desktop setups expose working EGL and /dev/dri devices but fail the
        # lightweight cuda-python probe. Fall back to the render card path directly.
        device = get_dri_card_by_index(index)
    print(device)
