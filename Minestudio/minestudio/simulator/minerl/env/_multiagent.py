...
def _resolve_max_resetting_env_count() -> int:
    raw = os.getenv("MINESTUDIO_MAX_RESETTING_ENV_COUNT", "").strip()
    if raw:
        try:
            value = int(raw)
            if value > 0:
            ...
            pass
    return max(4, os.cpu_count() // 12) if os.cpu_count() is not None else 20


MAX_RESETTING_ENV_COUNT = _resolve_max_resetting_env_count()
        ...
        while ok != 1:
            ...
            if reply is None:
                ...
                if num_retries > MAX_WAIT:
                    raise socket.timeout("Mission init failed: instance returned no reply.")
                logger.warning(
                    "Mission init got empty reply from %s; retrying (%s/%s).",
                    instance,
                    num_retries,
                    MAX_WAIT,
                )
