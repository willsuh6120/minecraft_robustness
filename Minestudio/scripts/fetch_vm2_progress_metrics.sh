...
usage() {
  cat <<'EOF'
usage:
  bash scripts/fetch_vm2_progress_metrics.sh <remote_run_dir> [local_dest_dir]

example:
  bash scripts/fetch_vm2_progress_metrics.sh \
    /home/willsuh1114/envgen2/Minestudio/outputs/evaluate_rocket/ppo_straight_O2_single_env_smoke_stable_vm_20260427_055533
...
if [[ -z "${LOCAL_DEST_DIR}" ]]; then
  LOCAL_DEST_DIR="${LOCAL_IMPORT_ROOT}/$(basename "${REMOTE_RUN_DIR}")_progress_vm2"
fi
