...
RUN_TAG="${RUN_TAG:-p_obstacle_maze_t6_main_local_fresh_$(date +%Y%m%d_%H%M%S)}"
...
find_latest_local_asset_dir() {
  find "${ROOT_DIR}/outputs/evaluate_rocket" \
    -maxdepth 3 \
    -type d \
    -path "*/p_obstacle_maze_t6_main_local_fresh_*/assets/*" \
    | sort \
    | tail -n 1
}

if [[ -z "${ASSET_DIR:-}" ]]; then
  ASSET_DIR="$(find_latest_local_asset_dir)"
fi
if [[ -z "${ASSET_DIR:-}" || ! -d "${ASSET_DIR}/eval_bank" ]]; then
  echo "No reusable local maze_t6 asset bank found. Set ASSET_DIR explicitly to an existing assets/<timestamp> dir." >&2
  exit 1
fi
