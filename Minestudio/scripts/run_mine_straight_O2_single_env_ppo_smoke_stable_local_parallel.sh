...
export TASK_GROUP_NAME="${TASK_GROUP_NAME:-straight_O2_single_env_smoke_stable}"
export OUT_ROOT="${OUT_ROOT:-${ROOT_DIR}/outputs/evaluate_rocket/ppo_straight_O2_single_env_smoke_stable_local_$(date +%Y%m%d_%H%M%S)}"

# Stable smoke defaults:
# - keep the same fixed world
# - increase collect data
# - skip updates with too few successful trajectories
# - make PPO updates more conservative
...
# The user has already validated that this local box can sustain high worker counts.
export COLLECT_WORKERS="${COLLECT_WORKERS:-10}"
export EVAL_WORKERS="${EVAL_WORKERS:-10}"
export FINAL_EVAL_WORKERS="${FINAL_EVAL_WORKERS:-10}"
