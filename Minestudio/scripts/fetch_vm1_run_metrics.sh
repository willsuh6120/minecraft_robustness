...
usage() {
  cat <<'EOF'
usage:
  bash scripts/fetch_vm1_run_metrics.sh <remote_run_dir> [local_dest_dir]

example:
  bash scripts/fetch_vm1_run_metrics.sh \
...
if [[ -z "${LOCAL_DEST_DIR}" ]]; then
  LOCAL_DEST_DIR="${LOCAL_IMPORT_ROOT}/$(basename "${REMOTE_RUN_DIR}")_metrics_vm1"
fi
