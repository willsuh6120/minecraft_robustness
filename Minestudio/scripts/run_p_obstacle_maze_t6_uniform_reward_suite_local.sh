...
RUN_TAG="${RUN_TAG:-p_obstacle_maze_t6_main_local_$(date +%Y%m%d_%H%M%S)}"
...
if [[ -z "${ASSET_DIR:-}" || ! -d "${ASSET_DIR}/eval_bank" ]]; then
  echo "No existing local maze_t6 asset bank found. Set ASSET_DIR explicitly to an existing assets/<timestamp> dir." >&2
  exit 1
fi
