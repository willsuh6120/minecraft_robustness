...
TASK="${TASK:-mine_coal}"
TASK_GROUP_PREFIX="${TASK_GROUP_PREFIX:-mine_o2_base_collect_eval_control}"
...
OUT_ROOT="${OUT_ROOT:-${ROOT_DIR}/outputs/evaluate_rocket/o2_base_collect_eval_control_repeats/$(date +%Y%m%d_%H%M%S)}"
...
usage:
  ...
  bash scripts/run_o2_base_bank_control_repeats.sh
...
echo "[o2-base-control] goal_protocol=${GOAL_PROTOCOL}"
echo "[o2-base-control] goal_spec=${GOAL_SPEC:-auto_goal_or_baked_task_group}"
...
run_repeat() {
  ...
  echo "[o2-base-control] start rep=${rep_tag} eval_bank base_seed=${base_seed}"
  "${XVFB_RUN}" -a -s "${XVFB_SCREEN_ARGS}" "${cmd[@]}"

  echo "[o2-base-control] start rep=${rep_tag} collect blocks=${BLOCKS} base_seed=${base_seed}"
  RUN_ROOT="${RUN_ROOT}" \
  BLOCKS="${BLOCKS}" \
  ITERATION="${ITERATION}" \
...
echo "[o2-base-control] done out_root=${OUT_ROOT}"
