...
TASK="${TASK:-mine_coal}"
RUN_TAG="${RUN_TAG:-p_obstacle_hard_v2_$(date +%Y%m%d_%H%M%S)}"
MASTER_ROOT="${MASTER_ROOT:-${ROOT_DIR}/outputs/evaluate_rocket/p_obstacle_hard_v2_calibration/${RUN_TAG}}"
...
P_OBSTACLE_VARIANT_SET="${P_OBSTACLE_VARIANT_SET:-hard_v2}"
...
generate_assets() {
  ...
  mkdir -p "${ASSET_ROOT}"
  log "generating P-hard-v2 auto-goal bank under ${ASSET_ROOT}"
  TASK="${TASK}" \
...
validate_and_render_geometry() {
  "${PYTHON_BIN}" - <<'PY' "${ASSET_DIR}" "${MASTER_ROOT}/hard_v2_geometry.md"
import json
import sys
...
if bad:
    raise SystemExit(f"Invalid P-hard-v2 geometry: {bad}")
...
write_summary() {
  "${PYTHON_BIN}" - <<'PY' "${MASTER_ROOT}" "${BASELINE_EPISODES}"
import json
import sys
from pathlib import Path
...
lines = [
    "# P-Hard-V2 Calibration Summary",
    "",
    f"root: `{root}`",
    f"asset_dir: `{next((root / 'assets').glob('*'), '')}`",
    f"geometry: `{root / 'hard_v2_geometry.md'}`",
    "",
]
...
video_manifest = root / "video_rocket2_3ep" / "annotated_video_manifest.txt"
...
log "geometry=${MASTER_ROOT}/hard_v2_geometry.md"
