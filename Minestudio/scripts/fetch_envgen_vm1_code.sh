...
echo "[fetch-envgen-vm1] project=${CLOUDSDK_CORE_PROJECT}"
echo "[fetch-envgen-vm1] instance=${INSTANCE_NAME}"
echo "[fetch-envgen-vm1] zone=${ZONE}"
echo "[fetch-envgen-vm1] external_ip=${VM_EXTERNAL_IP}"
echo "[fetch-envgen-vm1] remote=${REMOTE_USER}@${REMOTE_REPO_ROOT}"
echo "[fetch-envgen-vm1] sync_mode=${SYNC_MODE}"

bash "${ROOT_DIR}/scripts/fetch_gcp_code_delta.sh" "$@"
