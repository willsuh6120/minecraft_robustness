...
RUN_TAG="${RUN_TAG:-p_obstacle_maze_t6_mastery_late_layers_main_vm_$(date +%Y%m%d_%H%M%S)}"

SOURCE_RUN_ROOT="${SOURCE_RUN_ROOT}" \
RUN_TAG="${RUN_TAG}" \
EXPERIMENT_NAME="${EXPERIMENT_NAME:-main_waypoint_mastery_full16_late_layers}" \
ENV_REWARD_SCALE="${ENV_REWARD_SCALE:-1.0}" \
TRAINABLE_SCOPE="${TRAINABLE_SCOPE:-heads_last}" \
